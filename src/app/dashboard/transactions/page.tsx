          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-100">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">TX ID</th>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {txs.length === 0 && (
                  <tr><td colSpan={8} className="px-4 py-6 text-center text-gray-500">No transactions yet.</td></tr>
                )}
                {txs.map(tx => {
                  const art = sampleArtworks.find(a => a.id === tx.artworkId);
                  return (
                    <tr key={tx.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm text-gray-600 font-mono">{tx.id}</td>
                      <td className="px-4 py-3 text-sm text-gray-600">{new Date(tx.date).toLocaleString()}</td>
                      <td className="px-4 py-3 flex items-center gap-3">
                        <img src={art?.image || '/arts/art1.jpg'} alt={tx.artworkTitle} className="w-12 h-8 object-cover rounded" />
                        <div>
                          <div className="font-medium">{tx.artworkTitle}</div>
                          <div className="text-xs text-gray-500">by {art?.artistName}</div>
                        </div>
                      </td>
                      <td className="px-4 py-3">{tx.buyerName}</td>
                      <td className="px-4 py-3 text-right">{tx.currency} {tx.amount.toFixed(2)}</td>
                      <td className="px-4 py-3 text-sm text-gray-600 flex items-center gap-2"><CreditCard className="w-4 h-4 text-gray-400" />{tx.paymentMethod || 'card'}</td>
                      <td className="px-4 py-3"><StatusBadge status={tx.status} /></td>
                      <td className="px-4 py-3 text-right">
                        <button aria-label="View transaction" onClick={() => setViewTx(tx)} className="p-2 rounded-md hover:bg-gray-50"><Eye className="w-4 h-4 text-gray-600"/></button>
                      </td>
                    </tr>
                  );
                })}
