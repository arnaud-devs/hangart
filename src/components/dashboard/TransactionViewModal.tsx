      ...
      <div className="relative bg-white rounded-lg shadow-lg w-full max-w-2xl p-6 z-10">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-1">
            ...
            <div className="mt-3 text-sm text-gray-600">{tx.artworkTitle}</div>
            <div className="mt-1 text-sm font-medium">{tx.currency} {tx.amount.toFixed(2)}</div>
            <div className="mt-2 text-sm text-gray-600 flex items-center gap-2"><CreditCard className="w-4 h-4 text-gray-400" />{tx.paymentMethod || 'card'}</div>
          </div>
          ...
          <div className="md:col-span-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="text-sm font-medium">{tx.buyerName}</div>
              </div>

              <div>
                <div className="text-sm text-gray-500">Date</div>
                <div className="text-sm font-medium">{new Date(tx.date).toLocaleString()}</div>
              </div>

              <div>
                <div className="text-sm text-gray-500">Artwork ID</div>
                <div className="text-sm font-mono">{tx.artworkId}</div>
              </div>
            ...
            <div className="mt-4">
              ...
              <p className="mt-2 text-sm text-gray-700">No additional notes available for this transaction.</p>
            </div>
          </div>
        </div>
